...
native driver is built in release mode and offline before every run; it is not
a member of the release workspace.
...
metadata, configuration, raw latency samples, latency distribution, throughput,
client CPU, process peak RSS, Python traced-allocation peak, transferred bytes,
checksum, and fixture-observed connection reuse.
...
client CPU. Python allocation numbers come from an otherwise identical replay,
so `tracemalloc` does not distort the timed sample; they cover Python-traced
allocations, not the Rust allocator. Peak RSS is a whole-process high-water mark
and therefore includes imports and runtime setup. Native CPU and RSS are read
from `/proc` on Linux; unsupported platforms record `null`. Tiny runs can report
zero native CPU because Linux accounting is tick-granular.
